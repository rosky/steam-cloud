...
2. 启动应用，支持 IDE 直接启动和编译打包后启动。
	...
	2. 打包编译后启动：首先执行 `mvn clean package` 将工程编译打包，然后执行 `java -jar sentinel-core.jar`启动应用。
...
1. 访问 http://localhost:8080 页面，可以在左侧看到 Sentinel 应用已经注册到了控制台，单击 **流控规则** ，可以看到目前的流控规则为空。
