...
1. 增加配置，在 nacos-discovery-provider 项目的 /src/main/resources/application.properties 中添加基本配置信息
...
2. 启动应用，支持 IDE 直接启动和编译打包后启动。
	...
	1. IDE直接启动：找到 nacos-discovery-provider 项目的主类 `ProviderApplication`，执行 main 方法启动应用。
	2. 打包编译后启动：在 nacos-discovery-provider 项目中执行 `mvn clean package` 将工程编译打包，然后执行 `java -jar nacos-discovery-provider.jar`启动应用。
...
下面将分析 nacos-discovery-consumer 项目的代码，演示如何 RestTemplate 与 FeignClient。
...
1. 配置必要的配置，在 nacos-discovery-consumer 项目的 /src/main/resources/application.properties 中添加基本配置信息
...
1. 启动应用，支持 IDE 直接启动和编译打包后启动。
	...
	1. IDE直接启动：找到 nacos-discovery-consumer 项目的主类 `ConsumerApplication`，执行 main 方法启动应用。
	2. 打包编译后启动：在 nacos-discovery-consumer 项目中执行 `mvn clean package` 将工程编译打包，然后执行 `java -jar nacos-discovery-consumer.jar`启动应用。

#### 验证
1. 在浏览器地址栏中输入 http://127.0.0.1:18083/echo-rest/1234，点击跳转，可以看到浏览器显示了 nacos-discovery-provider 返回的消息 "hello Nacos Discovery 1234"，证明服务发现生效。

![rest](https://cdn.nlark.com/lark/0/2018/png/54319/1536986302124-ee27670d-bdcc-4210-9f5d-875acec6d3ea.png)

1. 在浏览器地址栏中输入 http://127.0.0.1:18083/echo-feign/12345，点击跳转，可以看到浏览器显示 nacos-discovery-provider 返回的消息 "hello Nacos Discovery 12345"，证明服务发现生效。
