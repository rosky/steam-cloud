...
1. Add necessary configurations to project `nacos-discovery-provider`, file /src/main/resources/application.properties.
...
2. Start the application in IDE or by building a fatjar.
	...
	1. Start in IDE: Find main class `ProviderApplication ` in project `nacos-discovery-provider`, and execute the main method.
	2. Build a fatjar：Execute command `mvn clean package` in project ` nacos-discovery-provider ` to build a fatjar，and run command `java -jar nacos-discovery-provider.jar` to start the application.
...
The code of `nacos-discovery-consumer` project will be analyzed below, demonstrating how RestTemplate and FeignClient.
...
1. Add necessary configurations to project `nacos-discovery-consumer` file /src/main/resources/application.properties.
...
1. Start the application in IDE or by building a fatjar.
	...
	1. Start in IDE: Find main class `ConsumerApplication ` in project `nacos-discovery-consumer`, and execute the main method.
	2. Build a fatjar：Execute command `mvn clean package` in project ` nacos-discovery-consumer ` to build a fatjar，and run command `java -jar nacos-discovery-consumer.jar` to start the application.

#### Verification
1. Enter `http://127.0.0.1:18083/echo-rest/1234` in the browser address bar and click Go to, we can see that the browser displays the message "hello Nacos Discovery 1234" returned by nacos-discovery-provider to prove that the service discovery is in effect.

![rest](https://cdn.nlark.com/lark/0/2018/png/54319/1536986302124-ee27670d-bdcc-4210-9f5d-875acec6d3ea.png)

1. Enter `http://127.0.0.1:18083/echo-feign/12345` in the browser address bar and click Go to, we can see that the browser displays the message "hello Nacos Discovery 12345" returned by nacos-discovery-provider to prove that the service discovery is in effect.
