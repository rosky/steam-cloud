...
1. Start in IDE: Find main class `GatewayApplication ` in project `nacos-gateway-discovery`, and execute the main method.
2. Build a fatjar：Execute command `mvn clean package` in project `nacos-gateway-discovery` to build a fatjar，and run command `java -jar nacos-gateway-discovery.jar` to start the application.
...
1. Start in IDE: Find main class `ProviderApplication ` in project `nacos-gateway-provider`, and execute the main method.
2. Build a fatjar：Execute command `mvn clean package` in project `nacos-gateway-provider` to build a fatjar，and run command `java -jar nacos-gateway-provider.jar` to start the application.
